...
public class NumberActivity extends Activity implements SimpleGestureListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		playAudio(Constants.NUMBERS_SOUND[position]);
		...
		imgNumber.setImageResource(Constants.NUMBERS_IMAGES[position]);
		...
		btnPrevious.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (position > 0) {
					...
					updateActivity((Constants.NUMBERS_IMAGES.length - 1));
					position = Constants.NUMBERS_IMAGES.length - 1;
				}
			}
		});
		...
		btnNext.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if (position < (Constants.NUMBERS_IMAGES.length - 1)) {
					updateActivity(position + 1);
					position = position + 1;
				} else {
					updateActivity(0);
	...
	public void onSwipe(int direction) {
		...
		case GF.SWIPE_RIGHT:
			...
			break;
		case GF.SWIPE_LEFT:
			if (this.position < (Constants.NUMBERS_IMAGES.length - 1)) {
				updateActivity(position + 1);
				position = position + 1;
			} else {
				updateActivity((Constants.NUMBERS_IMAGES.length - 1));
