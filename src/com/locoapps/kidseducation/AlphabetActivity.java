...
public class AlphabetActivity extends Activity implements SimpleGestureListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.alphabetactivity);
		playAudio(Constants.APHABET_SOUND[position]);
		...
		imgAlphabet.setImageResource(Constants.ALPHABET_IMAGES[position]);
		...
		btnPrevious.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (position > 0) {
					...
					position = Constants.ALPHABET_IMAGES.length - 1;
				}
			}
		});
		...
		btnNext.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if (position < (Constants.ALPHABET_IMAGES.length - 1)) {
					updateActivity(position + 1);
					position = position + 1;
				} else {
					updateActivity(0);
