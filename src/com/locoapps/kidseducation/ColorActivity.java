...
public class ColorActivity extends Activity implements SimpleGestureListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		playAudio(Constants.COLOR_SOUND[position]);
...
//	    mAdView.loadAd(adRequest);
		...
		imgColor.setImageResource(Constants.COLOR_IMAGES[position]);
		...
		btnPrevious.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (position > 0) {
					...
					position = Constants.COLOR_IMAGES.length - 1;
				}
			}
		});
		...
		btnNext.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				if (position < (Constants.COLOR_IMAGES.length - 1)) {
					updateActivity(position + 1);
					position = position + 1;
				} else {
					updateActivity(0);
